...
void gcr_compute(void) {
    ...
    if (usb_gcr_auto)  // If automatic GCR is enabled
    {
        ...
        } else if ((uint16_t)g_v_5v_avg > V5_HIGH)  // else if 5v bus is high and we have not reached the increase threshold
        {
            ...
            if (gcr_change_counter > 0) {
                gcr_change_counter--;
            } else if (gcr_change_counter < 0) {
                gcr_change_counter++;
            }
        }
    ...
    } else  // Else automatic GCR is disabled so immediately follow user's desired value
    {
        if (gcr_actual < gcr_use)
            action = ACT_GCR_INC;  // If actual has not met the use value, set action to increase
        else if (gcr_actual > gcr_use)
            action = ACT_GCR_DEC;  // Else if the actual is more than the use value, set action to decrease
    }
...
void flush(void) {
    ...
    if (gcr_actual != gcr_actual_last) {
        ...
        if (!I2C3733_Control_Get()) {
            i2c_led_q_run();
        }  // Run the queue if we know flush will not complete due to drivers being disabled
    }
#endif  // USE_MASSDROP_CONFIGURATOR
